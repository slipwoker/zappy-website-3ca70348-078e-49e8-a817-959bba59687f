...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (phoneHeaderBtn) {
    phoneHeaderBtn.addEventListener('click', function() {
      const phoneNumber = '+972502222494';
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
        ...
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '3ca70348-078e-49e8-a817-959bba59687f',
                name: formData.name,
                email: formData.email,
                subject: formData.service ? 'Contact Form - ' + formData.service : 'Contact Form Submission',
      ...
      alert('תודה שפנית אלינו! נחזור אליך בהקדם האפשרי.');
      contactForm.reset();
    });
  }
});
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "WILDCLEAR משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
